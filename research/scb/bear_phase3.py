#!/usr/bin/env python3
"""Bear Resolution Phase 3: 3 branches × 3 caps
Branch 1: G_both_12_7 (Bear specialist)
Branch 2: E_stock_12_7 + current theme (DD specialist)
Branch 3: F_stock_6_2 + current theme (Balanced)
"""
...
# Phase 3: 3 branches × 3 caps + baseline
_BRANCHES = {
    'A_current':   {'theme':'mom63+decel', 'stock':'alpha63_shrink'},
    'E_stk12_7':   {'theme':'mom63+decel', 'stock':'alpha_12_7'},
    'F_stk6_2':    {'theme':'mom63+decel', 'stock':'alpha_6_2'},
    'G_both12_7':  {'theme':'mom252_ex_recent', 'stock':'alpha_12_7'},
}
CAPS = [0.20, 0.25, 0.30]
VARIANTS = {}
for bname, bcfg in _BRANCHES.items():
    for cap in CAPS:
        vname = f'{bname}_c{int(cap*100)}'
        VARIANTS[vname] = {**bcfg, 'cap': cap}
...
for pos in range(len(rebal_idx)-1):
    ...
    for vname, vcfg in VARIANTS.items():
        ...
        if not port:
            results[vname].extend([0.0]*len(hold_dates));continue
        ws=w5b_w(port,cap=vcfg.get('cap',0.30))
...
eq_dates=tk_wide.index[-len(results[list(results.keys())[0]]):]
...
print(f'\n{"="*120}')
print(f'  BEAR RESOLUTION: Phase 3 Results (3 branches × 3 caps)')
print(f'{"="*120}')
...
# Delta vs baseline
print(f'\n  Delta vs A_current:')
a_s=calc_stats(results['A_current_c30'])
a_bear=calc_stats(np.array(results['A_current_c30'])[bear_mask.values[:len(results['A_current_c30'])]]).get('sharpe',0)
for vname in list(VARIANTS.keys()):
    if vname=='A_current_c30':continue
    dr=results[vname];s=calc_stats(dr)
    if not s:continue
    bs_arr=np.array(dr)[bear_mask.values[:len(dr)]]
    bear_s=calc_stats(bs_arr).get('sharpe',0) if len(bs_arr)>20 else 0
