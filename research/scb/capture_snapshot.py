"""PIT-safe snapshot capture script.
Run on each rebalance day (or daily) to archive theme membership + prices.
Usage: python3 capture_snapshot.py
"""
import json, pandas as pd, numpy as np
from pathlib import Path
from datetime import datetime

API = Path(__file__).resolve().parent.parent.parent / 'public' / 'api'
...
# === 2. Theme price snapshot (latest date from each theme) ===
prices = []
for f in sorted((API / 'theme-details').glob('*.json')):
    d = json.load(open(f))
    slug = d['slug']
...
print(f'  membership: {len(mem_df)} rows ({mem_df.theme.nunique()} themes, {mem_df.ticker.nunique()} tickers)')
print(f'  prices:     {len(price_df)} rows')
print(f'  ranking:    {len(rank_df)} rows')
print(f'  saved to:   {SNAP_DIR}/')
